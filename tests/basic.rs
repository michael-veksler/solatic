...
fn empty_clause_is_unsat() {
    let mut solver = Solver::new();
    if solver.add_clause([].as_slice(), 0).is_none() {
        return;
    }
...
fn nontrivial_sat() -> anyhow::Result<()> {
    ...
    solver.add_clause(&[-1, -2], 0).ok_or_else(unexpected)?; // v1 -> !v2
    solver.add_clause(&[2, 3], 0).ok_or_else(unexpected)?; // !v2 -> v3
    solver.add_clause(&[-3, 1], 0).ok_or_else(unexpected)?; // v3 -> v1

    // now make it impossible to assign !v1, v2, !v3, leaving only v1, !v2, v3 as a solution
    solver.add_clause(&[1, -2, 3], 0).ok_or_else(unexpected)?;
